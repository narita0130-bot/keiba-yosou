...
def combo_key(bet_type, combo):
    ...
    nums = [int(x) for x in combo.replace(">", "-").split("-") if x.strip()]
...
def main():
    ...
    for r in rows:
        ...
        hit = payouts.get(combo_key(kind, r["combo"]))
    ...
    if args.out:
        cols = list(rows[0].keys()) + ["finished", "hit", "payout_yen", "payout_ninki", "return", "profit"]
